...
void init_disasm() {
  void *dl_handle;
  dl_handle = dlopen("tools/capstone/repo/libcapstone.so.5", RTLD_LAZY);  // capstone is a disassembler for multi architecture
  assert(dl_handle);
...
void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
  ...
  Assert(count == 1, "str=%s, size=%d, pc=%lx, code=%p, nbyte=%d", str, size, pc, code, nbyte);
