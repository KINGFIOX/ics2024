...
static void decode_operand(Decode *s, uint8_t opcode, int *rd_, word_t *src1, word_t *addr, int *rs, int *gp_idx, word_t *imm, int w, int type) {
  switch (type) {
    ...
    case TYPE_Ib2E:
      ...
      break;
    case TYPE_SI2E:
      printf("rd_ = %d, addr = 0x%08x, gp_idx = %d\n", *rd_, *addr, *gp_idx);
      printf("rd_ = %s, addr = 0x%08x, gp_idx = %s\n", reg_name(*rd_, 4), *addr, reg_name(*gp_idx, 4));
      decode_rm(s, rd_, addr, gp_idx, w);
      imm();
      break;
    case TYPE_r:
...
again:
  ...
  INSTPAT("1000 1101", lea, SI2E, 0, printf("rd = %d, addr = 0x%08x, gp_idx = %d\n", rd, addr, gp_idx););
