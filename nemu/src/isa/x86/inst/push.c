...
void push(int width, word_t data) {
  ...
  vaddr_t vaddr = reg_read(R_ESP, width);  // get %esp, %esp is 4 bytes
  vaddr -= width;
  reg_write(R_ESP, width, vaddr);
  vaddr_write(vaddr, width, data);

  Assert(vaddr_read(reg_read(R_ESP, width), width) == data, "push error");
}

word_t pop(int width) {
  assert(width == 1 || width == 2 || width == 4);

  vaddr_t vaddr = reg_read(R_ESP, width);
