...
#define SEG16(type, base, lim, dpl)                                                                                                                    \
  (SegDesc){(lim) & 0xffff,         (uintptr_t)(base) & 0xffff, ((uintptr_t)(base) >> 16) & 0xff, type, 0, dpl, 1, (uintptr_t)(lim) >> 16, 0, 0, 1, 0, \
            (uintptr_t)(base) >> 24}

#define SEG32(type, base, lim, dpl)                                                                                                                    \
  (SegDesc){((lim) >> 12) & 0xffff, (uintptr_t)(base) & 0xffff, ((uintptr_t)(base) >> 16) & 0xff, type, 1, dpl, 1, (uintptr_t)(lim) >> 28, 0, 0, 1, 1, \
            (uintptr_t)(base) >> 24}

#define SEG64(type, dpl) (SegDesc){0, 0, 0, type, 1, dpl, 1, 0, 0, 1, 0, 0}

#define SEGTSS64(type, base, lim, dpl)                                                                                                             \
  (SegDesc){(lim) & 0xffff,        (uint32_t)(base) & 0xffff, ((uint32_t)(base) >> 16) & 0xff, type, 0, dpl, 1, (uint32_t)(lim) >> 16, 0, 0, 0, 0, \
            (uint32_t)(base) >> 24}

#define GATE32(type, cs, entry, dpl) (GateDesc32){(uint32_t)(entry) & 0xffff, (cs), 0, 0, (type), 0, (dpl), 1, (uint32_t)(entry) >> 16}

#define GATE64(type, cs, entry, dpl) \
  (GateDesc64){(uint64_t)(entry) & 0xffff, (cs), 0, 0, (type), 0, (dpl), 1, ((uint64_t)(entry) >> 16) & 0xffff, (uint64_t)(entry) >> 32, 0}
...
static inline void outb(int port, uint8_t data) { asm volatile("outb %%al, %%dx" : : "a"(data), "d"((uint16_t)port)); }

static inline void outw(int port, uint16_t data) { asm volatile("outw %%ax, %%dx" : : "a"(data), "d"((uint16_t)port)); }

static inline void outl(int port, uint32_t data) { asm volatile("outl %%eax, %%dx" : : "a"(data), "d"((uint16_t)port)); }
...
static inline void set_cr3(void *pdir) { asm volatile("mov %0, %%cr3" : : "r"(pdir)); }
...
static inline volatile BootRecord *boot_record() { return (BootRecord *)BOOTREC_ADDR; }
